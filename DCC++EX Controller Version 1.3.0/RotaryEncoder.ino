...
void rotaryEncoder() 
{
  ...
  REpinA = digitalRead(pinA);
  if (REpinA != lastREpinA  && REpinA == 1)
  {
    if (digitalRead(pinB) != REpinA){
      if (encoderPos >= 1){
        elapsedTime = (millis() - lastTime);    // Debounce Time
        encoderPos --;
        if (elapsedTime <= 100){
          if (encoderPos >= REAccAmount){
            encoderPos = (encoderPos - REAccAmount);    //decrement the encoder's position count
          }
        }
      ...
      if(encoderPos < highest){
        ...
        if (elapsedTime <= 100){
          if (encoderPos <= (highest - REAccAmount)){
            encoderPos = (encoderPos + REAccAmount);
          }
        }
