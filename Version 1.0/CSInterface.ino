...
void sendCMD(String CMD)
{
    ...
    if(wifiEnabled == 1)
    {
      if(!client.connect(host.c_str(), port))
      {
        ...
        nextionSetText("Progress", wifiStatus);
        Serial.println(wifiStatus);
      }else{
...
void parse(char *com)
{
  switch(com[0])
  {
    case 'p':
    {
      if(com[1] == '0') nextionSetValue("Power",0);
      if(com[1] == '1') nextionSetValue("Power",1);
      break;
    }
    default:
      break;
...
void updateWiFi()
{
    ...
    if(wifiEnabled == 1)
    {
      ...
      #endif
      nextionSetText("WiFiState", "WiFi Enabled...");
      wifiEnabled = 1;
    }else{
      #if defined WIFI
        WiFi.disconnect();
...
{
  nextionSetValue("throttle", speed);
  nextionSetValue("speedVal", speed);
}
