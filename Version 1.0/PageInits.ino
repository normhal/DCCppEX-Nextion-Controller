...
void initPage(uint8_t page)
{
  ...
  switch (page)
  {
    ...
    case LocoEditPage:
    {
      nextionSetText("A", String(readLocoAddress(editingID)));
      #if defined ESP
        if(readLocoAddress(editingID) == 0) nextionSetText("N","");
        else nextionSetText("N", readEEPROMName(locoNameBase + (editingID * (locoNameLen+1))));      //Get Loco Name from Integrated EEPROM
      #endif
      ...
      #endif
      #if defined AVR_UNO && defined USE_NEXTION_EEPROM
        nextionCommand(("repo N.txt," + String((editingID)*(locoNameLen+1))).c_str());               //Loco Name stored in Nextion EEPROM
      #endif
    ...
    case AccPage:                                                                                                                   
    {
      if (accStartID == 0) nextionCommand(F("PageDn.pic=PAGEDNGREYED"));
    ...
    case AccEditPage: 
    {
      ...
      #if defined ESP
        nextionSetText("N", readEEPROMName(accNameBase + (editingID * (accNameLen+1))));
      #endif
      #if defined AVR_UNO && !defined USE_NEXTION_EEPROM
        nextionSetText("N", readEEPROMName(accNameBase + (editingID * (accNameLen+1))));
      #endif
      #if defined AVR_UNO && defined USE_NEXTION_EEPROM
        nextionCommand(("repo N.txt," + String(accNameBase + (editingID * (accNameLen+1)))).c_str());
      #endif
      ...
      if(readAccAddress(editingID) != 0)
      {
        ...
        nextionSetValue(F("DCC"),1);
      }else{
        nextionCommand(F("ImageA.pic=23"));   //+ String(GREYED_BUTTON)).c_str());  //23
        ...
        nextionSetValue(F("DCC"),1);                                       //Default to DCC
      }
      eMode = 0;
      nextionSetValue(F("Edit"),0);
      break;
    }
    //******************************************************************************************************************************
    case RoutesPage:
    {
      if (routeStartID == 0) nextionCommand(F("PageDn.pic=PAGEDNGREYED"));
      accSelectMode = false;
      eMode = 0;
    ...
    case ProgramPage:
    {
      nextionSetValue(F("Main"), 0);
      nextionSetValue(F("Prog"), 0);
      progMode = 0;
      break;
    }
    //******************************************************************************************************************************
    ...
    {
      nextionSetText("ID", String(editingID));                                                        //Load ID
      #if defined ESP
        nextionSetText("N", readEEPROMName(locoNameBase + (editingID * (locoNameLen+1))));
      #endif
      #if defined AVR_UNO && !defined USE_NEXTION_EEPROM
        nextionSetText("N", readEEPROMName(locoNameBase + (editingID * (locoNameLen+1))));
      #endif
      #if defined AVR_UNO && defined USE_NEXTION_EEPROM
        nextionCommand(("repo N.txt," + String(locoNameBase + (editingID * (locoNameLen+1)))).c_str());    //Load Name
      #endif
      nextionSetText("A", String(readLocoAddress(editingID)));                                               //Load Address
    ...
    case InfoPage:
    {
      nextionSetText(F("Version"), Version);
      break;
    }  
    //******************************************************************************************************************************
...
void setPageButtons(uint8_t startID, uint8_t rowsPerPage, uint8_t numRows)
{
  if (startID == 0)
  {
    nextionCommand(F("PageUp.pic=24"));    // + String(PAGEUP)).c_str());     //24
    nextionCommand(F("PageUp.pic2=25"));   // + String(PAGEUPON)).c_str());  //25
    nextionCommand(F("PageDn.pic=29"));    // + String(PAGEDNGREYED)).c_str());   //29
    nextionCommand(F("PageDn.pic2=29"));    // + String(PAGEDNGREYED)).c_str());    //29
  }else{   
    nextionCommand(F("PageDn.pic=27"));      // + String(PAGEDN)).c_str());     //27
    nextionCommand(F("PageDn.pic2=28"));     // + String(PAGEDNON)).c_str());  //28
  }
  if (startID < (numRows - rowsPerPage))
  {
    ...
    nextionCommand(F("PageUp.pic=26"));     //+ String(PAGEUPGREYED)).c_str());   //26
    nextionCommand(F("PageUp.pic2=26"));    //+ String(PAGEUPGREYED)).c_str());  //26
  }  
}
