...
void locoEditPage(uint8_t button)
{
  ...
  switch(button)
  {
    ...
    case DoneButton:
    {
      ...
      wait(5);                                 //Wait for Nextion's sake...
      ...
      writeLocoRNum(editingID, tempStr.toInt());
        ...
        writeEEPROMName((locoNameBase + (editingID * (locoNameLen+1))), (tempStr + '\0'));
        EEPROM.commit();
      #endif
      ...
      #endif
      #if defined AVR_UNO && defined USE_NEXTION_EEPROM
        nextionCommand(("wepo N.txt," + String(locoNameBase + (editingID * (locoNameLen+1)))).c_str());
        delay(5);
      #endif
      initPage(originPage);
      break;
    ...
    case EditButton:                      //
    {
      ...
      {
        eMode = 1;
        nextionCommand(F("Edit.pic=165")); // + String(EDIT_ON));
      }else{
        eMode = 0;
        nextionCommand(F("Edit.pic=164")); // + String(EDIT_OFF));
      }
      break;
    }
    default:
