  ...
  switch(button)
  {
    case DoneButton:
    {
      if(eMode == 2)        //Route Select Mode Active
      {
        ...
        eMode = 0;
        nextionCommand(F("Select.pic=144"));  //SELECT_OFF = 144
      }
      initPage(originPage);
      break;
    }
    ...
    case SelectButton:                      //This button sets "Select" state Off and deletes any route in progress
    {
      ...
      {
        accSelectMode = true;
        nextionCommand(F("Select.pic=145"));      //SELECT_ON = 145
      }else{                                      //Select State currently ON and Button Pressed, so turn if OFF
        accSelectMode = false;                    //Only turn Route Select Mode OFF
        nextionCommand(F("Select.pic=144"));      //SELECT_OFF = 144
        for(uint8_t i = 0; i < accRowsPerPage; i++)       //Reset all the YELLOW marked Accessories
        {
    ...
    case PageUpButton:
    {
      if(accStartID < (numAccs - accRowsPerNextionPage))
      {
        ...
        nextionCommand(F("PageDn.pic=27"));    // + String(PAGEDN));    //27
      }else{
        nextionCommand(F("PageUp.pic=26"));      // + String(PAGEUPGREYED)); //26
      }
    ...
    case PageDownButton:
    {
      ...
      {
        accStartID = accStartID - (accRowsPerNextionPage);
        nextionCommand(F("PageUp.pic=24"));    // + String(PAGEUP));  ///24
      }else{
        nextionCommand(F("PageDn.pic=29"));      // + String(PAGEDNGREYED));    //29
      }
      accDrawPage(accStartID);
      break;
    }
...
void accDrawPage(uint8_t accStartID) 
{
  ...
  for(uint8_t accID = accStartID; accID < (accStartID + accRowsPerNextionPage); accID++)                      //Name, Address and Image must be updated
  {
    ...
    { 
      #if defined ESP
        nextionSetText("n" + String(slotNum), readEEPROMName(accNameBase + (accID * (accNameLen+1))));
      #endif
      #if defined AVR_UNO && !defined USE_NEXTION_EEPROM
        nextionSetText("n" + String(slotNum), readEEPROMName(accNameBase + (accID * (accNameLen+1))));
      #endif
      #if defined AVR_UNO && defined USE_NEXTION_EEPROM   
        nextionCommand(("repo n" + String(slotNum) + ".txt," + String(accNameBase + (accID * (accNameLen+1)))).c_str());  //Transfer the Accessory Name from Nextion EEPROM
      #endif
