...
void refreshThrottleValues(uint8_t activeSlot) 
{
  ...
  {
    nextionSetText("N", "Loco Slot Vacant");
    return;
  }
  ...
  for(uint8_t i = 0; i <numLocoSlots ; i++)
  {
    ...
    {
      nextionSetText("t" + String(i), String(readLocoAddress(selectedIDs[i])));
    }else{
      nextionSetText("t" + String(i), String(readLocoRNum(selectedIDs[i])));
    }
  }
}
/*
...
{
  #if defined ESP
    nextionSetText("N", readEEPROMName(locoNameBase + (selectedIDs[activeSlot] * (locoNameLen+1))));
  #endif
  #if defined AVR_UNO && !defined USE_NEXTION_EEPROM
    nextionSetText("N", readEEPROMName(locoNameBase + (selectedIDs[activeSlot] * (locoNameLen+1))));
  #endif
  #if defined AVR_UNO && defined USE_NEXTION_EEPROM
    nextionCommand(("repo N.txt," + String(selectedIDs[activeSlot] * (locoNameLen+1))).c_str());
  #endif
  nextionSetValue(F("speedVal"), (locos[selectedIDs[activeSlot]].speed));
  nextionSetValue(F("throttle"), (locos[selectedIDs[activeSlot]].speed));
  nextionSetValue(F("ForRev"), (locos[selectedIDs[activeSlot]].dir));
}
/*
 ***************************************************************************************************************
...
void setLocoTabs(uint8_t tabNumber)
{
  ...
  {
    nextionCommand(("Left.pic=" + String(tabBase + tabNumber)).c_str());
    nextionCommand(("Right.pic=" + String(RightNone)).c_str());
  }
  if(tabNumber >=5 && tabNumber <=9)    //Righthand Tabs
  {
    nextionCommand(("Right.pic=" + String(tabBase + tabNumber)).c_str());
    nextionCommand(("Left.pic=" + String(LeftNone)).c_str());
  }  
}
/*
 ***************************************************************************************************************
 * Function to Toggle a Function's state
 * The active LocoID is assumed
 ***************************************************************************************************************
 */
