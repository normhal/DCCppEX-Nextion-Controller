...
void setup() 
{
  ...
  nextionCommand(F("bkcmd=0"));    //Suppress Error details from the Nextion
...
{
  {
    message = nextionReceive();           //returns a String
    if (message != "") 
    {
      ...
      switch(button)
      {
        ...
        case WiFiRetryButton:
          if(wifiEnabled ==1)
          {
            ...
            nextionCommand((F("Sig.pic=") + String(wifiImage)).c_str());
...
void stopButton()
{
  ...
  updateNextionThrottle(locos[selectedIDs[activeSlot]].speed);
}
/*
 *************************************************************************************************************************
