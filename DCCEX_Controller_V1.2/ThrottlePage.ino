...
void throttlePage(uint8_t button)
{
  switch(button)                  
  {
    ...
    case SliderEvent:
    {
      int response = getNextionValue("S1");
...
void loadFunctions(uint8_t Page, uint8_t locoID)
{
  for(uint8_t l_fSlot=0; l_fSlot<10; l_fSlot++)       //10 function slots representing 2 uint8_ts per group and on a page - never more:-)
  {
    ...
    else    //fNum== 127
    {
      if (Page == LocoEditPage)
      {
        nextionCommand(("c" + String(l_fSlot)+".pco=65535").c_str());      //Colour to White
        nextionCommand(("n" + String(l_fSlot)+".pco=50712").c_str());      //Colour to Grey
