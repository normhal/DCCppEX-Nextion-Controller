  ...
  if(nextionPage == ThrottlePage)
  {
    ...
    { 
      if(selectedIDs[thNum][slot] != 255)
      {
        ...
        nextionSetValue(F("FR"), (locos[selectedIDs[thNum][slot]].dir));
...
void changeDirection()
{
  ...
  {        
    if(locos[selectedIDs[thNum][currentSlot]].dir == 1)       //Currently Forward
    {
      ...
      locos[selectedIDs[thNum][currentSlot]].dir = 0;
    ...
    {                                                   //Currently Reverse
      ...
      locos[selectedIDs[thNum][currentSlot]].dir = 1;
...
void setHeadingDetails(uint8_t activeSlot)
{
  if(selectedIDs[thNum][activeSlot] != 255)
  {
    nextionSetText("Na", readEEPROMName(locoNameBase + (selectedIDs[thNum][activeSlot] * (locoNameLen))));
    nextionSetText("Nb", readEEPROMName(locoTypeBase + (selectedIDs[thNum][activeSlot] * (locoTypeLen))));
    nextionSetText("Nc", String(readLocoRNum(selectedIDs[thNum][activeSlot])));
    nextionSetValue("S", locos[selectedIDs[thNum][activeSlot]].speed);
  }else
  {
    nextionSetText("Na", "");
    nextionSetText("Nb", "");
