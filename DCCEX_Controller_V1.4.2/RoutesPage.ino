...
void routesPage(uint8_t button)
{
    ...
    switch(button)
    {
      ...
      default:
      { 
        if(button < SlotCStart || (button > SlotCStart + routesPerPage)) break;           //filter out any false button values
        uint8_t slotNum = button - SlotCStart;                  //Save the Nextion Slot being edited
        if(eMode == 1)
        {
