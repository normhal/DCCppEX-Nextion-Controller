...
uint8_t connectWiFi(uint8_t retries)
{
    ...
    while(WiFi.status() != WL_CONNECTED) 
    {
      ...
      if(j>retries) 
      {
       nextionSetText("WiFiState", "Timeout Trying to Connect");
       return(1);
      }
    }
    wifiProgress = "Connected to AP";
    nextionSetText("WiFiState", wifiProgress);      //Now to connect to DCCEX...
    nextionSetText("RSSI", String(WiFi.RSSI()));
    ...
    if(WIFI_DEFINED != 1)
    {
      ...
      return;
        ...
        if(wifiEnabled == 1)
        {
          ...
          nextionSetText("Progress", "WiFi Disconnecting...");
        }
      #endif    
    }
  #endif    
