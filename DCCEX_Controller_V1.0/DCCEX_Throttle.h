...
*/
  ...
  #include <EEPROM.h>
  #include "Nextion.h"            //From Bently Born Library
  #include "Config.h"
...
//  #define NEXWAIT 20          //Used for Nextion getComponentText
  #define NEXWAIT 30          //Used for Nextion getComponentText
//  #define NEXWAIT 10          //Used for Nextion getComponentText
  ...
  //**************************************************************************************
    ...
    #if defined USE_NEXTION_EEPROM
      #define eepromEnd 1023              //Integrated Arduino UNO EEPROM
      #define numLocos 20                 //Multiples of 10
      const uint8_t numFSlots = 10;       //Function Slots per loco  - always 10 for this version
...
Nextion myNextion(nextion,38400);             //create a Nextion object named myNextion
...
typedef struct locomotive loco;
  ...
  uint8_t wifiEnabled = false;                //WiFi disabled by default to allow Credentials to be configured
...
#define routeLen 12     //2 bytes per object - ID and State
  ...
  const uint16_t locoRNumBase  = locoNameBase + (numLocos * (locoNameLen+1));
  ...
  const uint16_t accAddrBase   = accNameBase + (numAccs * (accNameLen+1));
...
//  const uint16_t configBase    = routeListBase + (numRoutes * routeLen);
  ...
  const uint16_t locoNameBase  = 0;
  const uint16_t accNameBase   = locoNameBase + (numLocos * (locoNameLen+1));
...
#define eeFunctionPulse (eepromEnd - 78)    //1023 - 78
#define eeRNumEnabled (eepromEnd - 77)      //1023 - 77
#define eeAccDelay    (eepromEnd - 76)      //1023 - 76 = 947  Delay between Accessory Route Commands
#define eewifiEnabled (eepromEnd - 75)      //1023 - 75 = 948
#define eeSSID        (eepromEnd - 74)      //1023 - 74 = 949  20 Max 20 characters
#define eePWD         (eepromEnd - 54)      //1023 - 54 = 969  20 Max 20 characters
#define eeIPAddr      (eepromEnd - 34)      //1023 - 34 = 989  26 Max 16 characters
#define eePort        (eepromEnd - 18)      //1023 - 18 = 1005  2 Max 2 uint8_ts
...
#define WiFiRetryButton 34

#define SliderButton 41
#define ThrottleButton 42

#define ConfigButton 44
#define WiFiButton 45
...
/*
 * Types of Text Data sent by the Nextion
*/
//#define LNTxt 1              //Full Loco Name
//#define IDTxt 2              //Loco ID number
//#define ADTxt 3              //Loco Address
/*
