...
void rosterDrawPage(uint8_t locoStartID)
{
  for(uint8_t r = 0; r < rowsPerNextionRosterPage; r++)
  {
    ...
    if (readLocoAddress(locoStartID+r)!=0){
      myNextion.setComponentText(addressSlot,String(readLocoAddress(locoStartID+r)));
      #if defined ESP
        myNextion.setComponentText("n" + String(r), readEEPROMName(locoNameBase + ((locoStartID+r) * (locoNameLen+1))));
      #endif
      #if defined AVR_UNO && !defined USE_NEXTION_EEPROM
        myNextion.setComponentText("n" + String(r), readEEPROMName(locoNameBase + ((locoStartID+r) * (locoNameLen+1))));
      #endif
      #if defined AVR_UNO && defined USE_NEXTION_EEPROM
        myNextion.sendCommand(("repo n" + String(r) + ".txt," + String((locoStartID+r) * (locoNameLen+1))).c_str());
      #endif
    }else{
      myNextion.setComponentText(addressSlot," ");
      myNextion.setComponentText("n"+String(r)," ");
    }
  }
