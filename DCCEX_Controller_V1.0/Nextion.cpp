...
Some small modifications made by Norman K Halland 
normhal@gmail.com
Changes are marked with "nkh"

1. Added simple detection of Arduino UNO or ESP8266 to select Software Serial
   otherwise use Hardware Serial. See lines 50-52 of "nextion.h"

2. Added two classes: setNextionValue and getNextionValue.
   See lines 109 to 104 and 166 to 179
   
   These two classes enable setting and getting values of Nextion variables which
   don't have the ".val" extension eg. sys0, sys1, sys2, tch0, tch1, tch2, tch3
   and possibly others. Some are read-only - all should be used with care.
   
   I find the getNextionValue("tch0")or("tch1") useful to determine if a button is 
   still being pressed.

   My changes are a demonstration of my very primitive knowledge of C++ - please
...
boolean Nextion::setNextionValue(String component, int value){        //nkh
  String compValue = component + "=" + value;                         //nkh
  sendCommand(compValue.c_str());                                     //nkh
  boolean acki = ack();                                               //nkh
  return acki;                                                        //nkh
}//set_nextion_value                                                  //nkh
...
unsigned int Nextion::getNextionValue(String component){                    //nkh
  String getValue = "get " + component;                                     //nkh Don't add the ".val" extension
    unsigned int value = 0;                                                 //nkh
  sendCommand(getValue.c_str());                                            //nkh
  uint8_t temp[8] = {0};                                                    //nkh
  nextion->setTimeout(20);                                                  //nkh
  if (sizeof(temp) != nextion->readBytes((char *)temp, sizeof(temp))){      //nkh
    return -1;                                                              //nkh
  }//end if                                                                 //nkh
  if((temp[0]==(0x71))&&(temp[5]==0xFF)&&(temp[6]==0xFF)&&(temp[7]==0xFF)){ //nkh
    value = (temp[4] << 24) | (temp[3] << 16) | (temp[2] << 8) | (temp[1]); //nkh
  }//end if                                                                 //nkh
  return value;                                                             //nkh
}//get_nextion_value */                                                     //nkh
...
//String Nextion::getComponentText(String component){
...
String Nextion::listen(){//returns generic
